...
int  longeur = 0,j,supp,cmp = -1, num_enreg =0,blc = 1, chmp = 0;
int ident[600] = {0};
int nb_classe = 5;
...
//*******************************************************
void insertion( TOVC *f, char * eleve)
{
    int i,j;
    int trouve=0;

    if(entete(f,1)==0) {aff_entete(f,1,1);}
    if(rechercher(f,eleve,&i,&j)==0)
    {
        Buffer buff ;
        liredir(f,i,&buff);
        ...
        while (k != m)
        {
           if(j < Tail_bloc)
           {
               if(k>=taille){  k=0;}
               c=buff.chaine[j];
               buff.chaine[j]=eleve[k];
               if((entete(f,3)==Tail_bloc) && (m==strlen(eleve)-1)){stop = 1;}
               if (((i != entete(f,1))|| (j<entete(f,3)))&&(stop==0))
               {
                  m++;
                  if (m>=taille){ m=0;}
                  eleve[m]=c;
               ...
               {
                   if((i==entete(f,1))&&(j>=entete(f,3)))
                    {
                        if(m==-1){ m=strlen(eleve)-1;}
                       stop=1;
                    }
               }
               j++;
...
int rechercher(TOVC *F,char *cle_rechercher, int *block, int *champ){
    char chaine[Tail_bloc];
    char cle_extrait[Tail_bloc];
       while (cmp<0 && num_enreg < entete(F,2)){

        blc = blc + ((longeur + chmp) / Tail_bloc);
        chmp = (chmp + longeur) % Tail_bloc ;
        extr_enr(F,chaine,blc,chmp,&longeur,&supp);

        extrer_cle(chaine,cle_extrait);
        if(!supp){
    ...
    if (cmp < 0 && num_enreg == entete(F,2) -1 ){
       blc = (blc*Tail_bloc + longeur + chmp) / Tail_bloc;
       chmp = (chmp + longeur ) % Tail_bloc ;
        *block = blc;
        *champ = chmp;
       return 0;
...
void afficher_fichier(TOVC *f)
{
 ...
 int CreerFichierInitiale(TOVC *F,char *NomFichier , char *FichierNoms , char *FichierPrenoms ,char *note0 ,char *note1 ,char *note2,char *note3,char *note4,char *note5 ){
  ...
  //Gnr_eleve(char *eleve,int annee,char classe,int i,char *Noms,char *Prenoms,char *Notes)

  //char fichier_noms[50],fichier_prenoms[50],fichier_noteP[50],fichier_note1[50],fichier_note2[50],fichier_note3[50],fichier_note4[50],fichier_note5[50];
  //scanf()
...
{
    //srand(time(NULL));
    char chaine1[50] = {"180004145mDib,Adel250572545fBoumaza,Wassila2315705"};
    char chaine2[50] = {"32fHamidi,Hanane270049132mRezig,Abd El Alim2302394"};
    char chaine3[50] = {"33mBelkadi,Sammy260238321mKorichi,Abdelhak"};
    char chaine[50],cle[50];
    char chaine4[50] = {"180004145mDib,Adel"};
    char chaine6[50] = {"270049132mRezig,Abd El Alim"};
    char chaine7[50] = {0};
    char chaine5[50] = {"abcde"};


    int longeur,supp;
    TOVC *F;
   F = ouvrir("FichierEtudiant", 'N');
  // Buffer buff;
   CreerFichierInitiale(F,"FichierEtudiant","Noms.txt","Prenoms.txt","note0.txt","note1.txt","note2.txt","note3.txt","note4.txt","note5.txt");
  // afficher_fichier(F);
   //aff_entete(F,1,1);
 /*  insertion(F,chaine4);
   insertion(F,chaine6);
//  supprimer_eleve(F,chaine5);
  /*liredir(F,2,&buff);
  printf("\n%s",buff.chaine);*/
  /*printf("\n[i]------------%d",entete(F,1));
  printf("\n[j]------------%d",entete(F,3));*/
  /*extr_enr(F,chaine5,1,27,&longeur,&supp);
  printf("\n l'enregistrement : %s \ la longeur : %d \n supprimer : %d",chaine5,longeur,supp);
  ...
  while(i<=600)
  {
    ...
    // fermer(F);
   /* generer_fichier_note("note0.txt",0);
    generer_fichier_note("note1.txt",1);
    generer_fichier_note("note2.txt",2);
    generer_fichier_note("note3.txt",3);
    generer_fichier_note("note4.txt",4);
    generer_fichier_note("note5.txt",5);
